...
function renderBlockMarkdown(resolved) {
  ...
  return resolved.items
    .map((item) => {
      const link = `[${escapeMarkdownLinkText(item.title ?? item.name)}](${item.url})`;
...
export function renderEntityCardHtml(item, { headingLevel = "h3", root = "article" } = {}) {
  ...
  const description = item.description
    ...
    : "";
  return `<${Root} data-hover-card class="entity-card"><a class="entity-card-hit" href="${escapeHtml(item.url)}" target="_blank" rel="noreferrer" aria-label="${escapeHtml(label)}"></a><${Body} class="entity-card-body">${renderCardMedia({ ...item, name: title })}<${Copy} class="entity-card-copy"><${Name} class="entity-card-name">${escapeHtml(title)}</${Name}>${description}</${Copy}></${Body}></${Root}>`;
}

function renderEntityInlineHtml(item) {
  return `<span class="entity-chip"><a href="${escapeHtml(item.url)}">${escapeHtml(item.name)}</a><span class="entity-chip-popover" aria-hidden="true"><span class="entity-chip-popover-panel">${renderEntityCardHtml(item, { root: "span" })}</span></span></span>`;
}
...
function renderInlineHtml(resolved, token, format = "card") {
  ...
  return resolved.items.map((item) => renderEntityInlineHtml(item)).join("、");
}
...
function renderBlockHtml(resolved, format) {
  ...
  if (format === "plain") {
    const items = resolved.items
      .map((item) => {
        const link = `<a href="${escapeHtml(item.url)}">${escapeHtml(item.title ?? item.name)}</a>`;
