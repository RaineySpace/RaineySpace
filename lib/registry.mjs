...
const KIND_CONFIG = {
  ...
  friend: {
    ...
    label: "friend",
    imageField: "icon",
    allowedFields: new Set(["name", "title", "url", "date", "description", "icon", "pinned"]),
  },
};
...
export function parseRegistry(kind, raw, options = {}) {
  ...
  for (const [id, definition] of Object.entries(raw)) {
    ...
    for (const field of (kind === FRIEND_KIND ? ["name", "title", "url"] : ["name", "url"])) {
    ...
    entities.push({
      ...
      title: kind === FRIEND_KIND && typeof definition.title === "string" ? definition.title.trim() : undefined,
