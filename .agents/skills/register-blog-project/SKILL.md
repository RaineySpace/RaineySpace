...
description: 根据用户提供的项目官网、产品页或代码仓库地址，无论项目是否已登记，都先从官方来源抓取并核实名称、链接、简介、封面和源码地址，再与当前博客对比。项目已存在时告知用户并询问是否更新，不存在时告知用户并询问是否新增；仅在确认后修改博客。用户说“添加项目”“更新项目信息”“把这个项目放到博客”“抓取项目信息后收录”或提供项目 URL 要求维护项目列表时使用。
---
...
- 不猜测项目定位、发布日期、源码仓库、许可、用户规模或商业状态。
...
打开用户提供的地址，并按需检查同域页面、页面元数据、结构化数据、品牌资源、官方文档和页面明确链接的源码仓库。必要时搜索以定位官方页面，但最终候选事实必须回到官方来源核验。
...
- “这是源码仓库地址，帮我收录为项目。” → 将仓库作为 `sourceUrl`，继续寻找并核实官方产品地址；找不到时把缺口交给用户确认。
